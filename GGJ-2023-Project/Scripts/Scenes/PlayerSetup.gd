...
@onready var _piggy_parent = $Piggies
...
func _ready():
	...
	for i in range(_piggy_parent.get_child_count()):
		_piggy_parent.get_child(i).set_color(PLAYER_COLORS[wrapi(i, 0, PLAYER_COLORS.size())])
...
func _on_player_count_changed():
	for i in range(_piggy_parent.get_child_count()):
		_piggy_parent.get_child(i).visible = i < PlayerManager.players
